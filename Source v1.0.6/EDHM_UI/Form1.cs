...
namespace EDHM_UI
{
	...
	public partial class Form1 : Form
	{
		...
		private void LoadIniSettings(string INI_FILE_PATH)
		{
			try
			{
				if (INI_FILE_PATH !=  string.Empty)
				{
					_Reader = new IniFile(INI_FILE_PATH); //<- Open and Read the INI file
		...
		private void SaveIniSettings(string INI_FILE_PATH)
		{
			try
			{
				if (INI_FILE_PATH !=  string.Empty)
		...
		private void cmdOpenFile_Click(object sender, EventArgs e)
		{
			try
			{
				...
				var ED_MODS_PATH = Util.WinReg_ReadKey("EDHM_UI", "ED_MODS_PATH");
				if (ED_MODS_PATH != null && ED_MODS_PATH.ToString() != string.Empty)
				{
					OFDialog.InitialDirectory = ED_MODS_PATH.ToString();
				}
				else
				{
				...
				{
					//Remembers the Path to easy future visits
					string path = System.IO.Path.GetDirectoryName(OFDialog.FileName); //<- Only the Folder Path
					Util.WinReg_WriteKey("EDHM_UI", "ED_MODS_PATH", path);
					...
					this.labelStatus.Text = string.Format("File Opened, {0:n1} kb Readed.", (file.Length / 1024));

					//Here we load the Options saved in the Ini File:
					LoadIniSettings(OFDialog.FileName);
				}
			}
			catch (Exception ex)
			{
		...
		private void cmdSaveSettings_Click(object sender, EventArgs e)
		{
			try
			{
				if (this.txtIniFile_Path.Text != string.Empty)
				{
					...
					{
						SaveIniSettings(this.txtIniFile_Path.Text);
					}
				}
				else
				{

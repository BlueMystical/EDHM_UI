...
export default {
    ...
    methods: {
        async OnInitialize(theme) {
            ...
            this.themeTemplate = theme;
            this.presets = theme.Presets;

            const defaultArea = { id: "Panel_UP", title: "Upper Panel" };
            this.loadProperties(defaultArea);
        },
        ...
        loadProperties(area) {
            ...
            if (this.themeTemplate != null && this.themeTemplate.ui_groups != null) {
                const areaIndex = this.themeTemplate.ui_groups.findIndex(item => item.Name === area.id); 
                this.dataSource = this.themeTemplate.ui_groups[areaIndex];
            }
        },
